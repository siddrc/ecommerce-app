ecommerceApp.controller('deleteMerchantProductsController', deleteMerchantProductsController);
deleteMerchantProductsController.$inject = ['$scope', '$http', '$state', 'permissions'];

function deleteMerchantProductsController(scope, http, state, permissions) {
    scope.products = {};
    scope.products.count = 0;
    permissions.checkPermissionOnThisPage("isDelete", "Products");
    ...
    scope.products.deleteProduct = function(productId) {
        if (confirm("Are you sure, you want to delete this product")) {
            http.delete("/ajax/merchant/products/" + productId).success(function(response) {
                showSuccessMessage(response.msg);
                scope.products.loadPage(1, 0, 12);
            }).error(function(errorResponse){
                showErrorMessage(errorResponse.msg);
            })
        }
