ecommerceApp.controller('landingPageController', landingPageController);
landingPageController.$inject = ['$scope', '$http', '$state', 'permissions'];

function landingPageController(scope, http, state, permissions) {
    scope.products = {};
    scope.products.count = 0;
    permissions.checkPermissionOnThisPage("isRead", "Products");
    http.get('/products/totalCount').success(function(products) {
    ...
    scope.products.loadPage = function(pageNumber, skip, limit) {
        ...
        http.get("/products?skip=" + skip + "&limit=" + limit).success(function(products) {
